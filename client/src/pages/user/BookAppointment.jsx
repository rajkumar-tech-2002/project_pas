...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Clock, CheckCircle, Info, MessageSquare } from 'lucide-react';
...
import { getAppointments, saveAppointment, getAvailableTimeRanges } from '../../services/appointmentService';
import { formatTime12Hour, formatDuration } from '../../services/utils/timeRangeUtils';
...
const BookAppointment = () => {
    ...
    const handleDateClick = async (date) => {
        ...
        const { availableRanges: ranges, availabilityWindows: windows } = await getAvailableTimeRanges(formattedDate);
        ...
        <div className="container-fluid px-4 fade-in">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p className="text-muted small mb-0">
                        Select a date from the calendar and choose your preferred time range for the Principal's office.
                    </p>
                </div>
            </div>
            ...
            <div className="row g-4">
                        ...
                        <div className="card-body p-4 p-md-5">
                            <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mb-5 gap-3">
                                <div className="d-flex align-items-center bg-light rounded-pill px-4 py-2 border border-light-subtle">
                                    ...
                                    <h6 className="mb-0 fw-bold">{format(currentDate, 'MMMM yyyy')}</h6>
                                </div>
                                <div className="d-flex align-items-center bg-white shadow-sm rounded-pill p-1 border">
                                    <button className="btn btn-link link-dark hover-bg-light p-2 rounded-circle border-0" onClick={handlePrevMonth}>
                                        <ChevronLeft size={20} />
                                    </button>
                                    <div className="vr mx-2 opacity-10"></div>
                                    <button className="btn btn-link link-dark hover-bg-light p-2 rounded-circle border-0" onClick={handleNextMonth}>
                ...
                <div className="col-lg-4">
                    <div className="card border-0 shadow-lg sticky-top" style={{ top: '100px', borderTop: '5px solid var(--primary-navy)' }}>
                        <div className="card-body p-4 p-md-4">
                            ...
                            {!selectedDate ? (
                                <div className="text-center py-5 bg-light rounded-4 border border-dashed border-2">
                                    ...
                                    <p className="small text-muted mb-0 fw-medium">Pick a date to begin</p>
                                </div>
                            ) : !selectedTimeRange ? (
                                <TimeRangeSelector
                                    date={selectedDate}
                                ...
                                <form onSubmit={handleBooking}>
                                    <div className="mb-4">
                                        <label className="form-label extra-small fw-bold text-muted text-uppercase tracking-widest">Selected Date</label>
                                        <div className="bg-light p-3 rounded-3 d-flex align-items-center border-start border-4 border-primary-navy">
                                            <CalendarIcon size={20} className="text-primary-navy me-3" />
                                            <span className="fw-bold fs-5">{selectedDate}</span>
                                        </div>
                                    </div>

                                    <div className="mb-4">
                                        <label className="form-label extra-small fw-bold text-muted text-uppercase tracking-widest">Confirmed Time</label>
                                        <div className="bg-light p-3 rounded-3 border-start border-4 border-accent-blue">
                                            <div className="d-flex align-items-center mb-2">
                                                ...
                                                <span className="fw-bold fs-5">
                                                    {formatTime12Hour(selectedTimeRange.start_time)} - {formatTime12Hour(selectedTimeRange.end_time)}
                                                </span>
                                            </div>
                                            <div className="extra-small text-muted">
                                                Duration: {formatDuration(
                                                    (new Date(`2000-01-01T${selectedTimeRange.end_time}`) -
                                                        new Date(`2000-01-01T${selectedTimeRange.start_time}`)) / 60000
                                                )}
                                            </div>
                                        </div>
                                        <button
                                    ...
                                    <div className="mb-4">
                                        <label className="form-label extra-small fw-bold text-muted text-uppercase tracking-widest">Meeting Agenda</label>
                            ...
                            <div className="mt-4 p-3 bg-primary-navy bg-opacity-5 rounded-3">
                                <h6 className="extra-small text-white fw-bold text-uppercase tracking-widest mb-2 opacity-75">Important Note</h6>
                                <p className="extra-small text-white mb-0">
                                    Appointments are subject to Principal's availability and may be rescheduled with prior notice.
                                </p>
                            </div>
                        </div>
                    </div>
